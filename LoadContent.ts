class LoadContent {
  drawGetComments() {
    ...
    while (localStorage.getItem(`dateAuthorComment${i}`) !== null && localStorage.getItem(`authorComment${i}`) !== null) {
      ...
      const newLikeLokalStorage: number = Number(localStorage.getItem(`newLike${newComment.getAttribute("data-index")}`));
      const likeLikalStorage: number = Number(localStorage.getItem(`like${newComment.getAttribute("data-index")}`));
      const btnPlus: HTMLButtonElement | null = authorAndMessage.querySelector(".button_minus");
    ...
    for (let i = 0; i <= messages.length; i++) {
      for (let k = 0; k <= messages.length; k++) {
        if (localStorage.getItem(`dateAnswer${i}.index${k}`) !== null && localStorage.getItem(`authorAnswer${i}.index${k}`) !== null && localStorage.getItem(`idAnswerParent${i}.index${k}`) !== null) {
          ...
          const authorAndMessage = document.createElement("div");
          authorAndMessage.className = "author_and_message";
          const avatarAuthor = <HTMLDivElement>document.getElementById("avatar")!.cloneNode(true);
          avatarAuthor.className = "avatar_author";
          const authorAndText = document.createElement("div");
          ...
          const newLikeAnswerLocalStorage: number = Number(localStorage.getItem(`newLikeAnswer${i}.index${k}`));
          const likeAnswerLocalStorage: number = Number(localStorage.getItem(`likeAnswer${i}.index${k}`));
  ...
  drawGetFavorites() {
    ...
    buttonFavorites.forEach(function (btn) {
      ...
      if (localStorage.getItem(`active-favorites${id}`) !== null && btn.closest(".answer") === null) {
        ...
        (btn as HTMLElement).style = "color: red";
